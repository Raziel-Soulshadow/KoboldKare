...
public class KoboldGenes {
    ...
    public KoboldGenes Randomize(float multiplier=1f) {
        ...
        SettingFloat variation1 = SettingsManager.GetSetting("RandomBrightnessVariation") as SettingFloat;           //these tighten or loosen the RandomGaussian
        float varAddition1 = 0f;
        varAddition1 = variation1.GetValue()/10f;
        SettingFloat variation2 = SettingsManager.GetSetting("RandomSaturationVariation") as SettingFloat;         
        float varAddition2 = variation2.GetValue()/10f;

        brightness = (byte)Mathf.RoundToInt(Mathf.Clamp(RandomGaussian(0f - varAddition1, 1f + varAddition1), 0, 1) * 255f);
        saturation = (byte)Mathf.RoundToInt(Mathf.Clamp(RandomGaussian(0f - varAddition2, 1f + varAddition2), 0, 1) * 255f);
    ...
    public static KoboldGenes Mix(KoboldGenes a, KoboldGenes b) {
        ...
        float varMulti = 0.7f;
        varMulti = (variation.GetValue() * -0.7f) + 0.7f;
        ...
        c.hue = (byte)Mathf.RoundToInt(FloatExtensions.CircularLerp(hueAngA, hueAngB, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1)) * 255f);
        c.brightness = (byte)Mathf.RoundToInt(Mathf.Lerp(a.brightness / 255f, b.brightness / 255f, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1)) * 255f);
        c.saturation = (byte)Mathf.RoundToInt(Mathf.Lerp(a.saturation / 255f, b.saturation / 255f, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1)) * 255f);
        //c.contrast = (byte)Mathf.RoundToInt(Mathf.Lerp(a.contrast / 255f, b.contrast / 255f, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1)) * 255f);
        //if using full random, use ^. Otherwise the memberwise clone will select the contrast randomly between the parents.
        c.bellySize = Mathf.Lerp(a.bellySize, b.bellySize, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.metabolizeCapacitySize = Mathf.Lerp(a.metabolizeCapacitySize, b.metabolizeCapacitySize, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.dickSize = Mathf.Lerp(a.dickSize, b.dickSize, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.ballSize = Mathf.Lerp(a.ballSize, b.ballSize, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.fatSize = Mathf.Lerp(a.fatSize, b.fatSize, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.baseSize = Mathf.Lerp(a.baseSize, b.baseSize, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.maxEnergy = Mathf.Lerp(a.maxEnergy, b.maxEnergy, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.dickThickness = Mathf.Lerp(a.dickThickness, b.dickThickness, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1));
        c.maxGrab = (byte)Mathf.Max(Mathf.RoundToInt(Mathf.Lerp(grab1, grab2, Mathf.Clamp(RandomGaussian(-0.2f + varMulti, 1.2f - varMulti), 0, 1))), 1);
